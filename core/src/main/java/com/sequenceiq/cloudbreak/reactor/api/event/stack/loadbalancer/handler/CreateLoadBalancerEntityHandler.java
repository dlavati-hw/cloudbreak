...
import com.sequenceiq.cloudbreak.reactor.api.event.stack.loadbalancer.UpdateActionsFinishedSuccess;
...
public class CreateLoadBalancerEntityHandler extends ExceptionCatcherEventHandler<CreateLoadBalancerEntityRequest> {
    ...
    protected Selectable doAccept(HandlerEvent event) {
        ...
        try {
            ...
            if (newLoadBalancers.isEmpty()) {
                LOGGER.debug("No load balancers generated. Check that correct entitlements are enabled and the environment has " +
                    "valid network settings. Ending flow.");
                return new UpdateActionsFinishedSuccess(request.getResourceId());
            } else {
                Stack savedStack;
